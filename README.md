...
В `manifest.json` есть флаг `"debug"`. Поставить `true` на хосте → на экране
вопроса рядом со счётчиком показывается уникальный `id` вопроса (напр. `m10`,
`l8`) — удобно сообщать, какой вопрос исправить. `false`/убрать — скрыто.
Включается/выключается подменой файла на сервере, без пересборки.
